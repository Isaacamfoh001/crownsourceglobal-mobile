 ...
 *
 * Which FOUR tabs render depends on the current Experience Mode
 * (`useExperience`, M32.4 §2/§3/§7) — never a new navigator, never a new
 * auth check: this is presentation only, and every underlying `vendor-*`
 * route/API call stays exactly as authorization-scoped as before. A vendor
 * with no FACTORY/BEAUTY eligibility (or whose stored mode preference
 * doesn't match a real capability) always falls back to the default
 * Seller tab set — see useVendorModeGuard for the actual membership gate.
 *
 * "More" (secondary functionality — Store settings, Messages,
 * Resolutions, Switch experience) is deliberately never a visible tab: it
 * stays registered here (so `router.push("/(vendor)/more")` from a
 * header button works) but permanently hidden via `href: null`, same
 * mechanism used to hide the other modes' tabs from each other.
 */
export default function VendorModeLayout() {
  ...
  return (
    <Tabs tabBar={(props) => <VendorTabBar {...props} />} screenOptions={{ headerShown: false }}>
      <Tabs.Screen name="index" options={{ title: "Dashboard", href: mode === "BEAUTY" ? null : undefined }} />
      ...
      <Tabs.Screen name="finance" options={{ title: "Earnings" }} />
