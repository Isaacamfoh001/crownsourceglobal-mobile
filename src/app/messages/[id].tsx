...
import { useConversationDetail, useReplyToConversation } from "@/features/messaging/useMessages";
import { friendlyErrorMessage } from "@/lib/api/errors";
import type { MessageDTO } from "@/types/api";

/**
 * Customer conversation thread (M30) — the native counterpart to
 * app/(customer)/account/messages/[id]/page.tsx. Operational support
 * messaging, not social chat (CLAUDE.md §8): no presence/typing/read-
 * receipt indicators, since none of that exists in the underlying
 * Message model.
 */
export default function MessageThreadScreen() {
  const { colors } = useAppTheme();
  const { id } = useLocalSearchParams<{ id: string }>();
  const query = useConversationDetail(id);
  const replyMutation = useReplyToConversation(id ?? "");
